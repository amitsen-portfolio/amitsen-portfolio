...
export default function Header() {
  ...
  return (
    ...
    <header className="z-[999] relative">
      ...
      <motion.div
        className="fixed top-0 left-1/2 h-[4.5rem] w-full rounded-none border border-white border-opacity-40 bg-white bg-opacity-80 shadow-lg shadow-black/[0.03] backdrop-blur-[0.5rem] sm:top-6 sm:h-[3.25rem] sm:w-[43rem] sm:rounded-full transition-all dark:bg-slate-950 dark:bg-opacity-75 dark:border-black/40"
            ...
            >
              ...
              <Link
                className={clsx("flex w-full items-center justify-center px-3 py-3 hover:text-orange-500 transition-all", {
                  "text-slate-950 dark:text-slate-200": activeSection === link.name,
                })}
              ...
              >
                {link.name}
                {
                  ...
                  activeSection === link.name && (
                    <motion.span className="bg-slate-200 rounded-full absolute inset-0 -z-10 dark:bg-slate-800"
